...
'''
    ...
    You can click on the notification window to speed things along.  The idea is that if you click while fading in, 
    you should immediately see the info. If you click while info is displaying or while fading out, the window closes 
    immediately.
...
image64_error = b'iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAMAAABEpIrGAAAAA3NCSVQICAjb4U' \
                b'/gAAAACXBIWXMAAADlAAAA5QGP5Zs8AAAAGXRFWHRTb2Z0d2FyZQB3d3cuaW5rc2NhcGUub3Jnm+48GgAAAIpQTFRF' \
                b'////20lt30Bg30pg4FJc409g4FBe4E9f4U9f4U9g4U9f4E9g31Bf4E9f4E9f4E9f4E9f4E9f4FFh4Vdm4lhn42Bv5GNx5W575nJ' \
                b'/6HqH6HyI6YCM6YGM6YGN6oaR8Kev9MPI9cbM9snO9s3R+Nfb+dzg+d/i++vt/O7v/fb3/vj5//z8//7' \
                b'+////KofnuQAAABF0Uk5TAAcIGBktSYSXmMHI2uPy8/XVqDFbAAAA8UlEQVQ4y4VT15LCMBBTQkgPYem9d9D' \
                b'//x4P2I7vILN68kj2WtsAhyDO8rKuyzyLA3wjSnvi0Eujf3KY9OUP+kno651CvlB0Gr1byQ9UXff' \
                b'+py5SmRhhIS0oPj4SaUUCAJHxP9+tLb/ezU0uEYDUsCc+l5' \
                b'/T8smTIVMgsPXZkvepiMj0Tm5txQLENu7gSF7HIuMreRxYNkbmHI0u5Hk4PJOXkSMz5I3nyY08HMjbpOFylF5WswdJPmYeVaL28968yNfGZ2r9gvqFalJNUy2UWmq1Wa7di/3Kxl3tF1671YHRR04dWn3s9cXRV09f3vb1fwPD7z9j1WgeRgAAAABJRU5ErkJggg=='

image64_success = b'iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAMAAABEpIrGAAAAA3NCSVQICAjb4U' \
                  b'/gAAAACXBIWXMAAAEKAAABCgEWpLzLAAAAGXRFWHRTb2Z0d2FyZQB3d3cuaW5rc2NhcGUub3Jnm+48GgAAAHJQTFRF' \
                  b''
...
def _display_notification(title, message, icon=image64_success,
    ...
    Displays a "notification window", usually in the bottom right corner of your display.  Has an icon, a title,
    and a message
    The window will slowly fade in and out if desired.  Clicking on the window will cause it to move through the end
    the current "phase". For example, if the window was fading in and it was clicked, then it would immediately stop
    ...
    window = sg.Window(title, layout, background_color=WIN_COLOR, no_titlebar=True, location=win_location,
                       keep_on_top=True, alpha_channel=0, margins=(0, 0), element_padding=(0, 0), grab_anywhere=True,
                       finalize=True)
...
def display_notification(title, message, icon=image64_success,
    ...
    :return: (Any) The Process ID returned from calling multiprocessing.Process
    """
    proc = Process(target=_display_notification,
                   args=(title, message, icon, display_duration_in_ms, fade_in_duration, alpha, location))
    proc.start()
    return proc
...
if __name__ == '__main__':
    ...
    proc3 = display_notification('Upper Left', 'This one does not fade in!', icon=image64_error, location=(0, 0),
                                 fade_in_duration=0)
